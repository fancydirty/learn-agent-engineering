...
O fallback é **governança de contexto**: gerencie ativamente o histórico que o loop produz em vez de empurrar cegamente para ele. As técnicas específicas — compactar turnos antigos, resumir resultados iniciais, descartar artefatos intermediários que já não importam — são o tema inteiro do Curso 5 desta série, "Agent Memory and State", então não vamos reabri-las aqui. O que você precisa levar do ponto de vista do loop é isto: governar o contexto é como você freia o custo por turno do loop, para que um loop que roda por muito tempo não fique progressivamente mais burro conforme avança.
