...
## Un bucle que corre también puede desbocarse
...
```agentmentor-check
{
  ...
  "prompt": "Tu agente ha llamado la misma herramienta de búsqueda 15 turnos seguidos. Cada turno la consulta es casi idéntica, y los resultados también — que están básicamente vacíos. El bucle sigue corriendo, el conteo de turnos sigue subiendo, y la factura también. ¿Qué salvaguarda deberías agregar primero a este arnés?",
  ...
  "choices": [
    ...
    {
      "id": "b",
      "text": "Cambiar a un modelo más grande y potente para que deje de correr la misma búsqueda una y otra vez",
      "correct": false,
    ...
    {
      "id": "c",
      "text": "Escribir un prompt de sistema más largo y detallado que le diga repetidamente que no corra búsquedas duplicadas",
      "correct": false,
...
- **Caso C:** Un agente trabaja en una tarea genuinamente compleja y corre con normalidad, pero como hay muchos turnos y cada turno acarrea un contexto grande, la factura nocturna de API llega muy por encima de lo esperado.
...
Alguien argumenta: «Ya puse un tope de turnos de 50. Con eso basta — por más desquiciado que se ponga el bucle, corre 50 turnos y se detiene. No hace falta ninguna detección de falta de progreso encima».
...
<!-- rubric -->
- Señala que un tope solo quema todos los turnos restantes (atascado en el turno 5 pero corriendo aún hasta el turno 50), desperdiciando tokens, tiempo y dinero
...
- Errores que se componen — la mayor parte de la entrada del modelo este turno es su propia salida del turno pasado, así que un paso equivocado se arrastra hacia adelante y se amplifica en cada turno posterior; combina eso con el hecho de que puede correr muchos turnos, y la autonomía viene empaquetada con costos más altos y la posibilidad de errores que se componen[^S1]. La compuerta es puntos de control más parada temprana.
