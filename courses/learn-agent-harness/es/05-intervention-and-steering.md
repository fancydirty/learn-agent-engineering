...
En código, es una bifurcación puesta delante de la línea que corre la herramienta:
  ...
  "choices": [
    {
      ...
      "correct": false,
      "feedback": "Eso confunde la trazabilidad posterior al hecho con la intercepción previa al hecho. La aprobación existe para impedir que una acción destructiva ocurra siquiera, y para cuando sale esta notificación, drop_table ya corrió contra producción — la tabla se fue, y revisarlo solo confirma la pérdida. Los logs y las notificaciones valen la pena, pero resuelven «podemos averiguar qué pasó», no «podemos impedir que pase»."
    },
    {
      "id": "b",
      "text": "No. La aprobación tiene que sentarse antes de que corra drop_table, con el asentimiento primero; un registro posterior al hecho da trazabilidad, no intercepción.",
      "correct": true,
...
3. El agente todavía corre tests cuando la persona mete un mensaje: «No publiques 2.3.1 — el cliente quiere 2.3.0, cambia a esa versión y continúa». El agente sigue con el nuevo número de versión.
...
<!-- hint -->
Hazte una pregunta primero: después de que esto pasó, ¿el bucle sigue corriendo? Si está completamente detenido, eso es una interrupción. Si sigue corriendo, mira si lo que cambió fue «el objetivo general del bucle» o «si una acción específica debería ocurrir».
...
<!-- hint -->
Vigila el orden de dos líneas: `executeTool` (de verdad envía el correo) y `askHuman` (le pregunta a una persona). ¿Cuál corre primero ahora mismo? La aprobación human-in-the-loop exige «aprobar primero, ejecutar segundo» — verifica si el orden está invertido.

<!-- hint -->
Voltear el orden no basta del todo por sí solo — cuando la aprobación no pasa, la herramienta de verdad no debe **correr** (usa `continue` para saltarla), y lo mejor es devolver la denegación al modelo como un `tool_result` con `is_error`, o bien puede proponer el mismo correo de nuevo el siguiente turno.
...
- Las tres intervenciones gobiernan cada una una capa distinta: **interrumpir** actúa sobre si el bucle vive (una persona cancela todo el bucle desde afuera), **dirigir** actúa sobre hacia dónde apunta (empuja una instrucción nueva, cambia el objetivo, sigue corriendo), **aprobar** actúa sobre si una acción pasa (pausa antes de una operación peligrosa y espera un asentimiento); la última de esas es exactamente la idea de «pausar para retroalimentación humana en puntos de control»[^S1] clavada delante de las acciones de alto impacto
