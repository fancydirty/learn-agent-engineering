...
- 긴 작업을 위한 컴팩션 전략을 설계하기: 무엇이 살아남아야 하는지(결정, 미해결 문제, 핵심 구현 세부)와 무엇을 버려도 되는지(중복된 도구 출력)를 판단하고, 구조화된 노트로 핵심 상태를 컨텍스트 윈도 밖에 세워 두기
...
- 컨텍스트 윈도가 유한한 자원이고 대화 이력은 늘어나기만 한다는 점을 안다(코스5)
