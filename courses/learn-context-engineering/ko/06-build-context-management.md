...
윈도 자체는 얼마나 클까요. 그것은 여러분 자신의 엔지니어링 판단으로 정의하는 상수입니다.
...
여기까지 와서, 이 코스가 레슨 1부터 레슨 6까지 가르친 모든 것 — 주의 예산, 시스템 프롬프트의 고도, 적시 검색, 컴팩션과 노트, 서브에이전트 격리 — 은 같은 결론 하나로 모입니다. 매 턴 모델이 보아야 할 것은 언제나 여러분이 거듭 되짚는 엔지니어링 판단이지, 한 번 정해 두고 잊는 설정이 아닙니다.
