...
**서킷 브레이커:** 에러율이 임계치를 넘으면 실패하는 서비스에 대한 호출을 일시적으로 멈추고 대신 빠르게 실패시켜 리소스 낭비를 막습니다.[^S7]
...
**장점:** 네트워크가 한 번 말썽을 부려 어떤 단계가 두 번 실행되더라도(첫 시도가 타임아웃됐지만 실제로는 성공했던 경우), 멱등성은 부수 효과가 중복되지 않도록 보장합니다.[^S9]
