...
当前是 2026 年 8 月。模型能力与 agent 框架相对年初已发生数量级变化，任何写作 agent 的训练知识都已过期。

1. **模型的内部知识不是真源，只是检索线索。** 子代理可以用自己的记忆判断"该去查什么"，但**任何写进课程的事实断言都必须来自本次写作时实时抓取的一手源**。禁止凭记忆写版本号、能力边界、默认行为、工具清单、价格、榜单名次。
2. **从第一性原理出发。** 优先教机制与判断（为什么会失败、如何验证、如何取舍），而不是教当下某个产品的操作步骤。机制的半衰期以年计，UI 步骤的半衰期以周计。
3. **每门课开写前必须重新抓源。** 即使本设计文档已列出来源，写作时仍须重新打开确认其当前状态；本文档中的来源列表是起点，不是缓存。
4. **凡易变必标版本边界。** 涉及规范版本、模型代号、SDK 行为、工具支持矩阵的句子，必须在正文写明"截至 YYYY-MM-DD / 版本 X"，并在 `topicCurrency.staleRisk` 说明何时会失效。
5. **不确定就不写。** 查不到一手证据的论断一律删除，不允许用"通常""一般来说"软化后保留。

可用的检索能力：Brave/Tavily/Exa 三家 API（key 在 `~/workspace/token.txt`）、`agent-browser`（有状态浏览器，可绕反爬）、`agent-reach`（多平台抓取）、WebFetch。
